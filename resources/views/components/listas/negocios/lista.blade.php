   ...
   @foreach ($negocios as $negocio)
      <li class="flex items-center justify-between gap-x-6 py-5">
         ...
         <div class="flex flex-none items-center gap-x-4">
            <a href="{{ route('negocio', ['id' => $negocio->uuid]) }}" class="hidden rounded-md bg-indigo-600 px-2.5 py-1.5 text-sm font-semibold shadow-xs inset-ring inset-ring-indigo-600 hover:bg-indigo-500 text-white sm:block">
               Entrar
               <span class="sr-only">, {{ $negocio->nombre }}</span></a>
            <el-dropdown class="relative flex-none">
               ...
               <el-menu anchor="bottom end" popover
                  ...
                  <a href="{{ route('negocio', ['id' => $negocio->uuid]) }}" class="block px-3 py-1 text-sm/6 rounded-sm focus:bg-indigo-500 focus:text-white focus:outline-hidden">Edit<span class="sr-only">, {{ $negocio->nombre }}</span></a>
                  <a href="#" class="block px-3 py-1 text-sm/6 rounded-sm focus:bg-red-500 focus:text-white focus:outline-hidden">Delete<span class="sr-only">, {{ $negocio->nombre }}</span></a>
               </el-menu>
            </el-dropdown>
         </div>
      </li>
