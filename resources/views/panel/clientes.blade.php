...
@section('drawers')
   <el-dialog id="modal_crear_cliente">
      <dialog id="drawer_crear_cliente" aria-labelledby="drawer-title" class="fixed inset-0 size-auto max-h-none max-w-none overflow-hidden bg-transparent not-open:hidden backdrop:bg-transparent">
               ...
               <div class="relative flex h-full flex-col overflow-y-auto bg-base-100 border-l border-base-content/20 py-6 shadow-xl">
                  <div class="px-4 sm:px-6">
                     <div class="flex items-start justify-between">
                        <h2 id="drawer-title" class="text-base font-semibold text-base-content">Crear cliente</h2>
                        <div class="ml-3 flex h-7 items-center">
                           <button type="button" command="close" commandfor="drawer_crear_cliente" class="relative rounded-md text-gray-400 hover:text-gray-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600">
                  ...
                  <div class="relative mt-6 flex-1 px-4 sm:px-6">
                     ...
                     <form id="crearCliente" action="{{ route('cliente.store') }}" method="POST" class="grid lg:grid-cols-4 grid-cols-1 gap-3">
                        ...
                        <div class="lg:col-span-full col-span-full">
                           ...
                           <div class="mt-2">
                              <input id="telefono" type="tel" name="telefono" autocomplete="telefono"
                                 class="block w-full rounded-md px-3 py-1.5 bg-base-200 text-base outline-1 -outline-offset-1 outline-base-content/20 placeholder:text-base-content/70 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
                           </div>
                        </div>
                        ...
                        <div class="lg:col-span-full col-span-full">
                           ...
                           <div class="mt-2">
                              <el-select id="select_negocio" name="negocio_id" value="{{ $negocios->first()?->uuid }}" class="mt-2 block">
                                 <button type="button"
                     ...
                     <script>
                        const crearClienteForm = document.getElementById('crearCliente');

                        crearClienteForm.addEventListener('submit', (e) => {
                           e.preventDefault();
                           peticion(crearClienteForm, {
                              resetForm: true,
   ...
   <script>
      function llamadaLista() {
         $.ajax({
            ...
            beforeSend: function() {
               $('#load_lista_clientes').empty().append(`
                  <li class="flex py-8">
                     <span class="mx-auto loading loading-spinner loading-md"></span>
                  </li>
               `)
            ...
            success: function(r) {
               document.getElementById('modal_crear_cliente').hide()
               $('#load_lista_clientes').empty().append(r.lista)
            },
            error: function(e) {
